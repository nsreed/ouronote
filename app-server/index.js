...
const ExistingFileType = _callable(
  class EFT extends Function {
    constructor() {
      let [flags] = arguments;
...
const PathType = _callable(
  class PT extends Function {
    constructor() {
      let [flags] = arguments;
      super("return arguments.callee.call.apply(arguments.callee, arguments)");
      Object.defineProperty(this, "name", {
...
commonParser.add_argument("-d", "--dry", {
  help: "Skips launching the server, instead shows config.",
  action: "store_const",
  dest: "next",
  const: showArgs,
});
...
httpsGroup.add_argument("--https", {
  action: "store_true",
  help: "Enable HTTPS Server",
  default: false,
});
httpsGroup.add_argument("--https-port", {
  type: Number,
  default: 443,
  help: "HTTPS Port",
});
httpsGroup.add_argument("--https-crt", {
  type: ExistingFileType("r"),
});
httpsGroup.add_argument("--https-key", {
  type: ExistingFileType("r"),
});
...
const commands = parser.add_subparsers({
  description: "Select a server configuration to run",
  required: false,
  dest: "subparser_name",
...
const localServerParser = commands.add_parser("local", {
  help: "Launches the local server configuration",
  description: "Local Server",
  parents: [serverParser, httpParser, httpsParser, logParser, commonParser],
});
...
function envToNamespace(parsersOrParser, env) {
  ...
  parsers = Array.isArray(parsersOrParser)
    ? parsersOrParser
    : [parsersOrParser];
  return parsers.reduce((allParsers, currentParser) => {
    const parserEnv = Object.entries(lowEnv).reduce((recognized, envEntry) => {
      ...
      const keyMatches = parserKeys.reduce((envObj, parserKey) => {
        ...
        const [ns, unrecognized] = currentParser.parse_known_args([
          optionString,
          `${envValue}`,
        ]);
...
function getHttps(app, args) {
  ...
  try {
    ...
    httpsServer = https
      .createServer(EXPRESS_OPTS, app)
      .listen(args.https_port, "0.0.0.0");
    return httpsServer;
  } catch (e) {
    throw e;
...
function startLocalServer(args) {
  ...
  // TODO FIXME This is always trying to launch HTTPS???????
  console.dir({ args });
...
const result = parser.parse_args(
  ...
  envToNamespace(parser, process.env)
);
console.log({ result });

const [current, ...remaining] = parser.parse_known_args(
  process.argv.slice(2),
  envToNamespace(parser, process.env)
);
